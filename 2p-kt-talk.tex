...
\begin{frame}[allowframebreaks]{Overview on the OOP Lib Design}
    ...
    \begin{enumerate}
        ...
        \item \alert{Aliases} can 
        %
        \begin{itemize}
            \item be inspected via \pl{alias/2}
            \item be added (resp. removed) via \pl{register/2} (resp. \pl{unregister/1})
            \item be used in place of actual \kt{Ref}erences via the \alert{de-aliasing operator} \pl{\$/1}
        \end{itemize}
    \end{enumerate}
...
\begin{frame}[allowframebreaks]{Architecture}
    ...
    \begin{center}
        \includegraphics[width=0.60\linewidth]{img/dsl-core.pdf}
    \end{center}
